package br.com.zenix.pvp.player.admin;
...
import br.com.zenix.pvp.player.admin.AdminManager.AdminLevel;

public class Vanish extends PvPListener {
	...
	public void updateVanished() {
		for (Player p : Bukkit.getOnlinePlayers()) {
			...
			if (getManager().getOneVsOneManager().getFight().isFighting(p.getUniqueId())
	...
	public void updateVanished(Player player) {
		for (Player p : Bukkit.getOnlinePlayers()) {
			if (!player.getName().equals(p.getName())) {
				...
				} else if (!getManager().getOneVsOneManager().getFight().isFighting(player.getUniqueId())
						|| !getManager().getGamerManager().getGamer(player).isSpectate()) {
					if (!player.canSee(p)) {
						if (!getManager().getOneVsOneManager().getFight().isFighting(player.getUniqueId())
								|| !getManager().getGamerManager().getGamer(player).isSpectate()) {
